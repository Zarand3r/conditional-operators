...
> **Audit, 2026-08-02: the table below did not reproduce, and is superseded.** Folding it into the
> paper required regenerating it from `results/*.json`, and three defects surfaced. **(a) The
> yardstick was inconsistent.** Some rows divide by the baseline the gate nominated in advance
> (dSprites, 3D Shapes, latent editing, both PDE rows), others by whichever baseline fits best
> (PDEBench). The choice matters: the PDE Complex FiLM row is `0.938` under the first rule and
> `1.122` under the second. **(b) The gap column mixes arms.** PDEBench's fit came from `hypernet`
> and its gap from `concat_mlp` (2815× vs 137×). The three rows reading `2.1×` are one genuine
> value (latent editing) copied to two others; recomputed they are `2.68×` (dSprites) and `13.11×`
> (3D Shapes). **(c) The LA-2A row quotes the narrow-FX screen.** `1.000 / 1.13×` are
> `audiofx_screen.json`, the 4-parameter synthetic chain, not the LA-2A. The correct table is now
> generated by `gen_tables.py::screen_table` into `docs/paper/tables/screen.tex`, using one rule:
> both quantities off the **best-fitting unstructured baseline**, since an arm cannot be its own
> yardstick.
>
> **What survives.** The separation survives; the threshold does not. Under the consistent rule the
> wins (`0.986`, `1.019`, `1.122`) still sit below the losses (`1.337`, `1.457`, `2.381`, `2.433`),
> but the dividing line moves from `1.05` to about `1.20`. The claim to make is that the two
> quantities separate the outcomes, not that `1.05` is a boundary.
