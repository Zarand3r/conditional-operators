...
| pdebench-reacdiff | Does any of this beat standard practice on a benchmark we did not build? | kill (gate said confirmed) | No. The gate passed on its own terms (+31.1% vs the best unstructured arm) but every arm of ours loses to plain concat_mlp: proposed -14.4%, cfilm_hyb -40.1%. The gate references film and hypernet, which are 4th and 8th of nine here; the strongest baseline is referenced by no criterion. Reported as a gate artifact, as the spec required |
| pdebench-split | Does containing the operator inside an expressive path beat standard practice? | kill (gate said confirmed) | No. Validation said +30.8% and +21.4% over concat_mlp; test says +1.7% (inside one sd) and -5.0%. Second gate artifact in a row. Containing the operator recovers the baseline and adds nothing |
| audiofx (screen) | Do parametric audio effects give the operator a home? | screen failed | No. First task where the bias MATCHES (proposed is the best-fitting arm, 1.000x) but the compositional gap is 1.13x, so there is nothing to generalise to. Larger steps raise train and test interaction equally; only split-shopping could manufacture a gap. Not pre-registered, no verdict claimed |
| la2a-cfilm | Does Complex FiLM beat FiLM on a real analog compressor, in that field's own setup? | kill | No. A tie with film (-2.8%, p=0.76) at 0.85x its parameters, and -17.1% to the strongest baseline. The winner is the hypernetwork at 10.7x film's params -- the opposite of capacity inversion, because this benchmark measures interpolation, not composition |
| composition-order | Does exact multiplicative aggregation beat the additive aggregation attention performs? | kill | Half. The advantage grows with the NUMBER of composed conditions exactly as predicted (+0.6% at n=1 to +46.6% at n=4, p=9.1e-5, delta=-1.00) and is absent at weak single conditions. It does NOT grow with strength -- it reverses beyond the trained scale, because rotations are periodic. The derivation ignored that |
| widefx (screen) | Does the advantage hold at dc=16, the regime the theory favours? | screen passed, prediction FAILED | Both registered checks passed (gap 1.83x, fit 1.011x) so the framework predicted a win; measured advantage flat at -1.1% across n=1,2,4,8. First out-of-sample test of the screen and it fails. The rigging check (0 of 12 pairs our form) called it beforehand and is the missing third condition |
...
Twenty gates in total: ten passed on their own terms, nine failed, one was inconclusive on budget. Two of the ten passes are recorded as gate artifacts: they cleared criteria that referenced the wrong baseline.
