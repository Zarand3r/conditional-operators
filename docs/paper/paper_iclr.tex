% CGA — unified ICLR submission draft. Combines paper_short.tex (framework, findings) and
% paper.tex (introduction, protocol, practical guidance). ICLR layout EMULATED: drop in the
% official iclr20XX_conference.sty at submission (unreachable from this machine); until then the
% preamble replicates its look (times, 10pt, 1in margins, review header).
...
\textbf{(3) Boundary map.} Five confirmations where conditions are transformations (up to five
orders of magnitude; capacity inversion in every image suite) and three mechanistic negatives
(fit, content, rollouts) that state exactly where operator guarantees stop
(\S\ref{sec:results}).
...
combinations lengthen. Learned angles match the true generators to $2{\times}10^{-4}$ rad, so
the operator is directly readable (Figure~\ref{fig:mech}).
...
\paragraph{(5) Three registered negatives.} On 3D~Shapes, CGA has the best OOD numbers
...
our contribution is the controlled decomposition at matched budget, plus a null that sharpens
it: fixed uniform contraction at any rate from $0.003$ to $0.1$ leaves horizon-20 error
unchanged, so the correction must be adaptive. That suite's registered gate scores KILL on a
separate single-step criterion, and we report it so.
...
established \citep{bradley2024cfg}; our deterministic instrument isolates it against an exact
alternative. CFG's error grows $50.7\times$ from strength 1 to 8; powering the condition
...
$2\times$, not the synthetic $10^5\times$. S4 shows orthogonality can price in-distribution fit
on rich scenes. The guidance instrument isolates distortion and cannot measure the
distribution-sharpening benefits CFG provides on multimodal targets. Conditions far from any
group action, such as open-ended text, remain untested; the learned-condition-geometry
extension is future work.

\paragraph{Reproducibility statement.} Every suite is pre-registered in committed
specifications; every number in this paper regenerates from append-only logs with one command
per suite; unit tests pin the operator invariants (identity at initialization and at $c=0$,
orthogonality, exact composition) and the budget counters; all amendments and one accounting
erratum are disclosed in the repository, which we release with the paper.
