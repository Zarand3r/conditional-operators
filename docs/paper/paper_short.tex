...
negatives tell practitioners exactly where the method stops working.
\end{abstract}
...
\textbf{Keep FiLM or adaLN when your conditions say what to generate.} Class labels, text
embeddings, identity. Our own registered negative shows rotation conditioning is $7\times$ worse
there. This is FiLM's main use today, and we recommend against swapping it.

\textbf{In rollouts, buy consistency, not composition.} All conditioners compounded error at the
same rate in our world-model suite. The bottleneck is whether the latent stays decodable, and a
slightly contractive transition handles that better than an exact isometry.
...
consistency, which no operator guarantee repairs. The first three point at one successor design:
the algebra torus $\times$ diagonal, which is complex-valued FiLM, with magnitude carrying
content and phase carrying composition. The fourth calls for a consistency objective on the
representation. Related work has learned group actions on latents for world models
