...
\end{minipage}
\caption{\textbf{Left (S6, S6$'$):} error against rollout length, trained only to length 3.
Without a consistency loss every conditioner drifts at much the same rate. Adding one separates
them: the isometric operator flattens out while the baselines keep climbing. \textbf{Right
(S8):} what turning guidance up does to the generated image. Both distort, since the target here
...
\end{minipage}
\caption{\textbf{Left (S2):} what happens when a test combination is longer than anything in
training. Every baseline gets worse; CGA does not move, because composition is a property of its
parametrization rather than something it had to learn. \textbf{Right (S1):} the angles the model
