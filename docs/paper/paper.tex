...
training distribution equally well, isolating composition. On dSprites, CGA reduces error on unseen
combinations by $53.8\%$ against the strongest baseline, at $39\times$ fewer conditioning FLOPs;
on the categorical variant, which adds a categorical shape factor, by $42.9\%$. The same result in a second form:
going from trained to unseen combinations multiplies CGA's error by $1.25\times$ (dSprites) and
$1.63\times$ (the categorical variant), and every baseline's by $2.1$--$3.3\times$. We call that multiplier the
